...
In virtual machine switch off the firewall and and trying pinging from your main computer by writing the following line in cmd 
ping (Virtual machine ip address) -t
If You are able to ping then in the virtual machine open PowerShell ISE and write the following code 
...
In the place of the API key you need to write your own api key which you can get from the link given .After you are done with it run the code and and you will get information of the attacker .All the attackers data will be stored in a c drive the link is given in the code itself open that and it will be saved in a text file . Copy the text file onto your main computer .

After that go to microsoft azure and in log anaytics workspace make a new script ,first link this text file to log ananlytics and then write a new script there as given below 
...
The first line will be name of the file how u have saved it .
Then you can go to azure sentinel and then in that you can even create a map by using the same script as above 
